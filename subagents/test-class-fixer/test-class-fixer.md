...
Normalize the input into a comma-separated list to pass to the test script (e.g., `ClassA,ClassB,ClassC`).
...
```
node ~/.cursor/scripts/run-apex-tests.js [ClassNames]
```

The script runs `sf apex run test` internally and returns only the test summary and any failed/skipped test methods (with their message and stack trace). Parse the JSON output to extract failure messages, stack traces, and assertion errors.
...
After successful deployment, re-run only the same test classes (not the full org test suite):
```
node ~/.cursor/scripts/run-apex-tests.js [ClassNames]
```
